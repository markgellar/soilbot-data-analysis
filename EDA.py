...
clean_data = "SSURGO_results.csv"
SSURGO_folder = "PA071"
base = Path(__file__).parent / SSURGO_folder
spatial = base / "spatial"

""" 
I'm going to use Kriging to build
a spatially autocorrelated map of CEC, sand%,
clay%, and silt% values.
These are crucial (I think) to predicting
true pH values and buffer pH
"""

soilmap = gpd.read_file(spatial / "soilmu_a_pa071.shp")

soilmap_projected = soilmap.to_crs(epsg=32618)

soilmap_projected["centroid_lon"] = soilmap_projected.geometry.centroid.to_crs(epsg=4326).x
soilmap_projected["centroid_lat"] = soilmap_projected.geometry.centroid.to_crs(epsg=4326).y

centroids = (
    soilmap_projected.groupby("MUKEY")[["centroid_lon", "centroid_lat"]]
    .mean()
    .reset_index()
)

centroids.rename(columns={"MUKEY": "mukey"}, inplace=True)
centroids["mukey"] = pd.to_numeric(centroids["mukey"], errors="coerce")


df = pd.read_csv(clean_data)
df["mukey"] = pd.to_numeric(df["mukey"], errors="coerce")
df = df.merge(centroids, on="mukey", how="left")
